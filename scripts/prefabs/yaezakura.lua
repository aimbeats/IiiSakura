...
local assets = {
	Asset( "ANIM", "anim/yaezakura.zip" ),
	Asset( "ANIM", "anim/ghost_yaezakura_build.zip" ),
}
...
local common_postinit = function(inst) 
	-- 小地图图标
	inst.MiniMapEntity:SetIcon( "yaezakura.tex" )
	inst:AddTag("yaezakura")
	--未知
	inst:AddTag("yaezakura_builder")
end
--定义八重樱能力倍率变量
local statedata = 
{
...
local function become(inst,state)
	...
	if state == "disintegration" then
		...
		-- inst.AnimState:SetBuild("yaezakura_rage")
	elseif state == "half_disintegration" then
		inst.components.talker:Say("进入半崩坏状态")
		-- inst.AnimState:SetBuild("yaezakura_half_disintegration")
	elseif state == "rage" then
		inst.components.talker:Say("进入狂暴状态")
		-- inst.AnimState:SetBuild("yaezakura_disintegration")
	else
		inst.components.talker:Say("进入正常状态")
		-- inst.AnimState:SetBuild("yaezakura")
	end
end
--判断理智范围触发能力修改的函数和标签
...
local master_postinit = function(inst)
	...
	--注册按钮监听组件
	-- inst:AddComponent("yaezakura_keyhandler")
...
STRINGS.CHARACTER_TITLES.yaezakura = "八重樱"
STRINGS.CHARACTER_NAMES.yaezakura = "八重樱"
STRINGS.CHARACTER_DESCRIPTIONS.yaezakura = "守护一世的巫女，却失去最应守护的珍爱"
STRINGS.CHARACTER_QUOTES.yaezakura = "我想守护的人都已不在"
STRINGS.CHARACTERS.YAEZAKURA = require "speech_yaezakura"

return MakePlayerCharacter("yaezakura", prefabs, assets,common_postinit,master_postinit,start_inv)