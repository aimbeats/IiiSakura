local assets=
{ 
    Asset("ANIM", "anim/stealth_dachi.zip"),--这个是放在地上的动画文件
    Asset("ANIM", "anim/swap_stealth_dachi.zip"), --这个是手上动画
    Asset("ATLAS", "images/inventoryimages/stealth_dachi.xml"),--物品栏图标的xml
}
...
local function OnEquip(inst, owner) --当你把武器装备到手上时，会触发这个函数
    owner.AnimState:OverrideSymbol("swap_object", "swap_stealth_dachi", "swap_stealth_dachi")--这句话的含义是，用swap_myitem_build这个文件里的swap_myitem这个symbol，覆盖人物的swap_object这个symbol。swap_object，是人物身上的一个symbol，swap_myitem_build，则是我们之前准备好的，用于手持武器的build，swap_myitem就是存放手持武器的图片的文件夹的名字，mod tools自动把它输出为一个symbol。
...
local function fn()--这个函数就是实际创建物体的函数，上面所有定义到的函数，变量，都需要直接或者间接地在这个函数中使用，才能起作用
    ...
    inst.components.equippable.walkspeedmult = 11.5--设置持有时的移动速度
...
STRINGS.NAMES.stealth_dachi = "无形"
